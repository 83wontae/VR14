...
void AShootingHUD::BindMyPlayerState()
{
	...
	if (IsValid(pc))
	{
		...
		if (IsValid(ps))
		{
			...
			ps->Fuc_Dele_UpdateSh.AddDynamic(this, &AShootingHUD::OnUpdateMySh);
			OnUpdateMySh(ps->CurSh, ps->MaxSh);
...
void AShootingHUD::OnUpdateMySh_Implementation(float CurSh, float MaxSh)
{
}
